...
import com.cg.eshop.util.OrderConstants;
...
public class OrderProductCrudController {
	...
	public SuccessMessage addOrderProduct(@PathVariable("custId") Integer custId)
	...
	public List<OrderProducts> viewOrderByUserId(@PathVariable("custid") Integer custId)
			throws OrderProductsNotFoundException, CustomerNotFoundException {
		return orderProductService.viewOrderByUserId(custId);

	}

	@GetMapping("vieworderdetails/{orderId}")
	public List<OrderedProductDetails> viewOrderDetails(@PathVariable("orderId") Integer orderId)
			throws OrderProductsNotFoundException {
